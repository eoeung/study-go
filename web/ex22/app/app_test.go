...
func TestTodos(t *testing.T) {
	...
	// test를 실행하면, 구글 로그인이 필요하기 때문에 구글 로그인을 건너뛰고 테스트를 진행
	// → 실제로는 로그인이 안됐지만, 로그인이 완료됐다는 가정 하에 진행할 수 있도록 해야함
	// → dummy 데이터를 만들어야함
	// 같은 app 패키지에 선언되어있으므로, 값만 바꿔주면 됨
	getSessionID = func(r *http.Request) string {
		return "testsessionId" // 로그인 체크시 getSessionID가 여기에 작성한 값으로 바뀜 → func 포인터이므로
	}
